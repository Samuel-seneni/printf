	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) &7 padd == ' ')/* extra char to left of buff */
		{
			if (extra_c)
	...
	int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		{
			return (write(1, &buffer[0]), + write(1, &buffer[ind, length);
		}
	}
	return (write(1, &buffer[ind], length));
}
...
* @flags: Flags
* @width: width
* @extra_c: char reprenting extra char
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (extra_c)
		buffer[--ind] = extra_c;
	return (write1, &buffer[ind], BUFF_SIZE - ind -1);
}
